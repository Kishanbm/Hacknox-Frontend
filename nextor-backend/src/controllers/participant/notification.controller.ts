...
export const getNotificationsController = async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
        ...
        console.log('[Notifications] Fetching for userId:', userId, 'role:', userRole, 'hackathonId:', hackathonId);
        
        const rawNotifications = await getRelevantNotifications(userId, userRole, hackathonId);
        console.log('[Notifications] Raw notifications count:', rawNotifications.length);
        ...
        // with the public content fetched above.
            ...
            const announcementIds = rawNotifications.map((n: any) => n.id).filter(Boolean);
        ...
        console.log('[Notifications] Final notifications count:', notifications.length);
...
export const markNotificationReadController = async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
        ...
        if (markAll) {
            ...
            await Promise.all(notificationIds.map(id => markNotificationRead(userId, id, hackathonId)));
