...
export const handleSubmissionDraft = async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
        ...
        if (hackathonId) {
            const { data: hackathon } = await supabase.from('Hackathons').select('submission_deadline').eq('id', hackathonId).maybeSingle();
            const deadline = hackathon?.submission_deadline;
            if (deadline) {
                const now = new Date();
        ...
        const submission = await createSubmission(submissionData, hackathonId);
...
export const getSubmissionDetails = async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
        ...
        // 1. Fetch submission first (without hackathon filter to allow viewing across hackathons)
        const submission = await getSubmissionById(submissionId, undefined);
        if (!submission) return res.status(404).json({ message: 'Submission not found.' });

        console.log('[Submission Details] Submission status:', submission.status, 'User role:', req.user!.role);

        // 2. If submission is public (submitted), allow any authenticated user to view (case-insensitive)
        if (submission.status && submission.status.toLowerCase() === 'submitted') {
            return res.status(200).json({ submission });
        }

        // 3. For non-public submissions, only allow admin/judge or team members
        ...
        console.error('[Submission Details Error]:', error.message);
        return res.status(500).json({ message: "Server Error", error: error.message });
    }
};
...
export const updateSubmissionDetailsController = async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
        ...
        // C. Call Service
        const updatedSubmission = await updateSubmissionDetails(submissionId, details.teamId, updates, hackathonId);
