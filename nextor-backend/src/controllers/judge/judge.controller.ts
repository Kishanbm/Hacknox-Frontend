...
import { getJudgeEvents as getJudgeEventsService } from "../../services/judge/judge.service";
...
export const getAssignedTeamsController = async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
        ...
        const { teams, totalCount } = await getAssignedTeams(judgeId, page, limit, hackathonId);
...
export const getJudgeDashboardController = async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
        ...
        const summary = await getJudgeDashboardSummary(judgeId, hackathonId);
...
export const getSubmissionForEvaluationController = async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
        ...
        const submission = await getSubmissionForEvaluation(judgeId, teamId, hackathonId);
...
export const saveEvaluationDraftController = async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
        ...
        const assignmentId = await getAssignmentId(judgeId, teamId, hackathonId); // <-- NEW CALL
        ...
        // 3. Call Service
        const savedDraft = await saveEvaluationDraft(judgeId, teamId, submission.id, assignmentId, payload, hackathonId);
...
export const getEvaluationDraftController = async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
        ...
        const evaluation = await getEvaluationByTeamId(judgeId, teamId, hackathonId);
...
export const submitFinalEvaluationController = async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
        ...
        const existingEvaluation = await getEvaluationByTeamId(judgeId, teamId, hackathonId);
        ...
        const finalEvaluation = await submitFinalEvaluation(judgeId, teamId, submission.id, payload, hackathonId);
        ...
        try {
            ...
            const adminMod: any = await import('../../services/admin/admin.service');
            if (adminMod && typeof adminMod.aggregateJudgeScores === 'function') {
                await adminMod.aggregateJudgeScores(hackathonId);
            }
        } catch (aggErr: any) {
            console.error("Aggregation Error [submitFinalEvaluationController]:", aggErr?.message ?? String(aggErr));
...
export const updateSubmittedEvaluationController = async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
        ...
        // 3. Call Service
        const updatedEvaluation = await updateSubmittedEvaluation(judgeId, teamId, payload, hackathonId);
...
export const getMyReviewsController = async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
        ...
        // 1. Call Service
        const reviews = await getMyReviews(judgeId, hackathonId);
...
export const getJudgeEventsController = async (req: AuthenticatedRequest, res: Response): Promise<any> => {
    try {
        const judgeId = req.user!.id;

        const events = await getJudgeEventsService(judgeId);
