 ...
 */
export const createJudgeAccount = async (email: string, firstName: string, lastName: string, hackathonId?: string | null): Promise<any> => {
    ...
    // Note: JudgeAssignments table requires team_id (links judge to team, not hackathon directly)
    // Judges will be assigned to teams later through the assignments UI
    console.debug(`[createJudgeAccount] Judge ${user.id} created. Hackathon context: ${hackathonId || 'none'}`);
 ...
 */
export const getJudgesList = async (page: number, limit: number, hackathonId?: string): Promise<any> => {
    const offset = (page - 1) * limit;
    console.debug('[getJudgesList] page=', page, 'limit=', limit, 'hackathonId=', hackathonId);

    // Fetch all judges with their profile
    ...
    // Fetch all assignments (optionally filtered by hackathon if provided)
    let assignmentsQuery = supabase
        .from('JudgeAssignments')
        .select('judge_id, team:Teams!inner(id, hackathon_id)');

    if (hackathonId) {
        assignmentsQuery = assignmentsQuery.eq('team.hackathon_id', hackathonId);
    }

    const { data: assignmentsAll, error: assignmentAllErr } = await assignmentsQuery;

    if (assignmentAllErr) {
        console.warn('Could not fetch assignment counts:', assignmentAllErr.message);
    }

    // Build map: judgeId -> count of assignments (in the specified hackathon if provided)
...
export const updateJudgeAccount = async (judgeId: string, payload: any): Promise<any> => {
    ...
    if (Object.keys(userUpdate).length > 0) updates.push(supabase.from('Users').update(userUpdate).eq('id', judgeId));
    if (Object.keys(profileUpdate).length > 0) updates.push(supabase.from('Profiles').update(profileUpdate).eq('user_id', judgeId));

    if (updates.length === 0 && !isPasswordResetting) return { message: 'No fields provided for update.' };
    ...
    if (isPasswordResetting) {
        const { data: currentUser, error: getUserErr } = await supabase.from('Users').select('email').eq('id', judgeId).single();
        if (getUserErr) console.error('Service Error [updateJudgeAccount - getUser]:', getUserErr.message);
        if (currentUser?.email) {
            const resetToken = crypto.randomBytes(32).toString('hex');
            const redisKey = `reset_token:${resetToken}`;
            try {
                await redisClient.setex(redisKey, 3600, judgeId);
            } catch (rErr: any) {
 ...
 */
export const getTeamsListAdmin = async (page: number, limit: number, filters: any, search: string, hackathonId?: string): Promise<any> => {
    const offset = (page - 1) * limit;

    let query = supabase
        .from("Teams")
        .select("id, name, city, verification_status, created_at,\n            leader:Users!leader_id (\n                profile:Profiles (first_name, last_name)\n            ),\n            memberCount:TeamMembers(count)", { count: 'exact' });
    ...
    const formattedTeams = (teams || []).map((team: any) => {
        ...
        return {
            ...
            memberCount: team.memberCount?.[0]?.count || 0,        
        };
    });
 ...
 */
export const getTeamProfileAdmin = async (teamId: string, hackathonId?: string): Promise<any> => {
 ...
 */
export const updateTeamAdmin = async (teamId: string, payload: any, hackathonId?: string): Promise<any> => {
    ...
    const { data: updatedTeam, error } = await query.select().single();
 ...
 */
export const verifyTeamAdmin = async (teamId: string, action: 'approve' | 'reject', adminId: string, hackathonId?: string): Promise<any> => {
    ...
    if (action === 'approve') {
        newStatus = 'verified';
        notificationMessage = "Congratulations! Your team has been successfully verified for the event.";
    } else if (action === 'reject') {
        newStatus = 'rejected';
        notificationMessage = "Important: Your team has been marked as rejected. Please contact support.";
    } else {
        throw new Error("Invalid verification action specified.");
    }

    // 1. Execute Update to change verification status
    ...
    if (updateError || !updatedTeam) {
        console.error("Service Error [verifyTeamAdmin]:", updateError?.message);
        throw new Error("Failed to update team verification status.");
    }

    // 2. Fetch team members for notification
    const { data: members, error: memberError } = await supabase
        .from("TeamMembers")
        .select('user:Users!user_id(id,email,is_active)')
        .eq("team_id", teamId);
    ...
    if (memberError || !members || members.length === 0) {
        ...
        const emailPromises = recipientEmails.map((email: string) => 
            sendEmail({
                ...
                html: "<h3>" + notificationMessage + "</h3>" +
...
export const getJudgeAssignments = async (hackathonId?: string): Promise<any> => {
    ...
    if (hackathonId) {
        const { data: judges, error } = await supabase
            .from('Users')
            .select('id, email, is_active, profile:Profiles!inner(first_name, last_name), assignments:JudgeAssignments!inner(id, team:Teams!inner(id, name, verification_status), team_id, hackathon_id)')
            .eq('role', 'judge')
        ...
        const assignmentMatrix = (judges || []).map((judge: any) => {
            ...
            return {
                ...
                judgeName: profile ? ((profile.first_name || '') + ' ' + (profile.last_name || '')).trim() : null,
    ...
    let query: any = supabase
        .from('Users')
        .select('id, email, is_active, profile:Profiles!inner(first_name, last_name), assignments:JudgeAssignments!judge_id(id, team:Teams!inner(id, name, verification_status), team_id, hackathon_id)')
        .eq('role', 'judge')
        .eq('is_active', true);
...
export const getSubmissionsPanel = async (page: number, limit: number, filters: any, hackathonId?: string): Promise<any> => {
    ...
    let query = supabase
        .from("Submissions")
        .select('id, status, submitted_at, repo_url, team:Teams!inner(id, name, verification_status), _evaluations:Evaluations(count)', { count: 'exact' });
...
export const getSubmissionDetailAdmin = async (submissionId: string, hackathonId?: string): Promise<any> => {
    ...
    const { data: submission, error } = await supabase
        .from("Submissions")
        .select('id, status, submitted_at, repo_url, description, zip_storage_path, team:Teams!inner(id, name, verification_status, project_category, hackathon_id), reviews:Evaluations(id, status, comments, score_innovation, score_feasibility, score_execution, score_presentation, judge:Users!judge_id(profile:Profiles(first_name, last_name)))')
        .eq("id", submissionId)
        .maybeSingle();
...
export const changeSubmissionStatus = async (submissionId: string, newStatus: string, adminNote?: string, hackathonId?: string): Promise<any> => {
    ...
    console.debug('[changeSubmissionStatus] update request:', { submissionId, newStatus, adminNote, hackathonId });
    const { data: updatedSubmission, error: updateError } = await supabase
        ...
        .eq("id", submissionId)
        .select('id, status, title, team:Teams!inner(id, leader_id, name, members:TeamMembers(user_id))')
        .maybeSingle();

    console.debug('[changeSubmissionStatus] supabase update result error:', updateError ? updateError.message : null);
    console.debug('[changeSubmissionStatus] supabase updatedSubmission:', updatedSubmission);
...
export const generateSubmissionDownloadUrl = async (submissionId: string, hackathonId?: string): Promise<string> => {
    ...
    const { data: submission, error: fetchError } = await supabase
        .from("Submissions")
        .select('zip_storage_path, team:Teams!inner(hackathon_id)')
        .eq("id", submissionId)
        .single();
...
export const computeFinalLeaderboard = async (hackathonId?: string): Promise<any> => {
    ...
    let query = supabase
        .from("AggregatedScores")
        .select('team_id, average_score, team:Teams!inner(name, verification_status, submission:Submissions(submitted_at), hackathon_id)')
...
export const getInternalLeaderboard = async (
        ...
        filters: any = {}, 
        // `isPublishedFilter` can be true/false to explicitly filter, or undefined to return all
        ...
        let query = supabase
        .from("Leaderboard")
        .select('rank, final_score, is_published, computed_at, team:Teams!inner(id, name, project_category)', { count: 'exact' }) // Ensure we count the total rows
        // Always order by rank
        .order("rank", { ascending: true });

    // --- 1. Apply Filters ---

    if (typeof isPublishedFilter === 'boolean') {
        // Apply explicit published/unpublished filter when provided
...
export const publishLeaderboardToggle = async (shouldPublish: boolean, adminId?: string, hackathonId?: string): Promise<any> => {
    ...
    const { data, error } = await query.select('team_id, rank, is_published');
 ...
 * Retrieves a paginated list of announcements for the Admin dashboard.
 */
export const getAnnouncementsList = async (page: number = 1, limit: number = 10, hackathonId?: string): Promise<any> => {
    const offset = (page - 1) * limit;
    ...
    const { data: announcements, error, count: totalCount } = await query
        .range(offset, offset + limit - 1);
 ...
 */
export const getAnalyticsOverview = async (hackathonId?: string): Promise<any> => {
    // 1. Fetch RAW status columns (lightweight)
    // Conditionally apply hackathon scoping to each query
    const teamsQuery: any = supabase.from("Teams").select("verification_status");
    if (hackathonId) teamsQuery.eq('hackathon_id', hackathonId);
    const { data: teams, error: teamError } = await teamsQuery;

    const submissionsQuery: any = supabase.from("Submissions").select("status");
    if (hackathonId) submissionsQuery.eq('hackathon_id', hackathonId);
    ...
    if (hackathonId) {
        ...
        if (subsErr) {
            ...
            const submissionIds = (subs || []).map((s: any) => s.id).filter(Boolean);
 ...
 */
export const getAnalyticsDetail = async (filters: { city?: string, college?: string, dateRange?: { start: string, end: string }, hackathonId?: string }): Promise<any> => {
    let query = supabase
        .from("Teams")
        .select("id, name, verification_status, city, college, \n            submission:Submissions!inner(submitted_at),\n            evaluations:Evaluations(id) "); // Removed !team_id alias constraint for cleaner join
 ...
 */
export const getStateCollegeBreakdown = async (hackathonId?: string): Promise<any> => {
 ...
 */
export const getAuditLogs = async (page: number = 1, limit: number = 20, hackathonId?: string): Promise<any> => {
    const offset = (page - 1) * limit;

    let query: any = supabase
        .from("AuditLogs")
        .select("id, action, details, created_at,\n            admin:Users!admin_id(email)", { count: 'exact' })
        .order('created_at', { ascending: false });

    // If hackathonId is provided, filter by the dedicated column.
    try {
        if (hackathonId) query = query.eq('hackathon_id', hackathonId);
        ...
        console.error("Service Error [getAuditLogs]:", err.message || err);
        throw new Error("Failed to retrieve audit logs.");
    }
...
export const exportTeamsData = async (filters: any): Promise<any[]> => {
    ...
    let query = supabase
    .from("Teams")
    .select("id, name, verification_status, project_category,\n        city, college, created_at,\n        members:TeamMembers(\n            user:Users!inner(\n                email, role, is_active,\n                profile:Profiles(\n                    first_name, last_name, phone\n                )\n            )\n        )");
 ...
 */
export const deleteJudgeAccount = async (judgeId: string, type: 'soft' | 'hard' = 'soft'): Promise<any> => {
    if (!judgeId) throw new Error('Judge ID is required.');

    if (type === 'soft') {
        const { error } = await supabase.from('Users').update({ is_active: false }).eq('id', judgeId);
        if (error) {
    ...
    const results = await Promise.all([
        supabase.from('JudgeAssignments').delete().eq('judge_id', judgeId),
        supabase.from('Profiles').delete().eq('user_id', judgeId),
        supabase.from('Users').delete().eq('id', judgeId),
    ]);

    const errors = results.map((r: any) => r.error).filter(Boolean);
    if (errors.length > 0) {
        console.error('Service Error [deleteJudgeAccount/hard]:', errors.map((e: any) => e.message));
...
/**
 * Create a simple audit log entry. `details` may be an object and will be
 * stored as JSON in the `details` column.
 */
export const logAdminAudit = async (adminId: string | null, action: string, details: any, hackathonId?: string | null): Promise<any> => {
    try {
        const payload: any = {
        ...
        const { data, error } = await supabase.from('AuditLogs').insert(payload).select();
        if (error) {
            console.warn('[logAdminAudit] failed to insert audit log:', error.message);
            return null;
        }
        return data?.[0] || null;
    } catch (e: any) {
        console.warn('[logAdminAudit] unexpected error:', e.message || e);
        return null;
    }
};