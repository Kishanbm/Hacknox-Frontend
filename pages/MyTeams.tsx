    ...
    <DashboardLayout>
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h1 className="text-3xl font-heading text-gray-900">Team Command</h1>
                <p className="text-gray-500">Manage squads, review invites, and access history.</p>
            </div>
            
            <div className="flex items-center gap-3">
                 <button 
                    onClick={() => navigate('/dashboard/teams/join')}
                    className="bg-white border border-gray-200 text-gray-700 px-5 py-2.5 rounded-xl font-bold hover:bg-gray-50 transition-colors flex items-center gap-2"
                 >
                    <Hash size={20} /> Join via Code
                </button>
                <button 
                    onClick={() => navigate('/dashboard/teams/create')}
                    className="bg-primary text-white px-5 py-2.5 rounded-xl font-bold hover:bg-primary/90 transition-colors flex items-center gap-2 shadow-lg shadow-primary/20"
                >
        ...
        {/* Tab Navigation */}
        <div className="flex items-center gap-1 bg-white p-1 rounded-xl border border-gray-100 w-fit">
            <button 
            ...
            {activeTab === 'active' && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {activeTeams.map((team) => (
                        <div key={team.id} className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md transition-shadow group">
                            {/* Header */}
                            <div className="p-6 border-b border-gray-50 bg-gray-50/50 flex justify-between items-start">
                                <div onClick={() => navigate(`/dashboard/teams/${team.id}`)} className="cursor-pointer">
                                    <div className="flex items-center gap-3 mb-1">
                                        <h3 className="text-xl font-bold text-gray-900 group-hover:text-primary transition-colors">{team.name}</h3>
                                    ...
                                    <div className="text-sm text-gray-500 font-medium flex flex-col gap-0.5">
                                ...
                                <div className="flex gap-2">
                                     <span className="px-2 py-1 bg-gray-200 text-gray-600 rounded text-xs font-bold self-start">{team.role}</span>
                    ...
                    <div 
                        onClick={() => navigate('/dashboard/teams/create')}
                        className="border-2 border-dashed border-gray-200 rounded-3xl flex flex-col items-center justify-center p-12 text-center hover:bg-gray-50 transition-colors cursor-pointer group min-h-[400px]"
                    >
                        <div className="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center text-gray-400 mb-4 group-hover:bg-primary group-hover:text-white transition-colors">
                            <Plus size={32} />
                        </div>
                    ...
                    {invites.length > 0 ? (
                        <div className="space-y-4">
                            {invites.map(invite => (
                                <div 
                                    key={invite.id} 
                                    className={`bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6 hover:shadow-md transition-all duration-300 relative overflow-hidden ${
                                ...
                                >
                                    ...
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-2 mb-1">
                                                ...
                                                <span className="text-xs font-bold text-gray-500 uppercase tracking-wide">{invite.role}</span>
                                            </div>
                                    ...
                                    <div className="flex gap-3 shrink-0">
                                        <button 
                                            ...
                                            className="px-6 py-2.5 bg-white border border-gray-200 text-gray-600 rounded-xl font-bold hover:bg-gray-50 hover:text-red-500 transition-colors"
                                        >
                                            Decline
                                        </button>
                                        <button 
                                            onClick={() => handleInviteAction(invite.id, 'accept')}
                                            className="px-6 py-2.5 bg-primary text-white rounded-xl font-bold hover:bg-primary/90 shadow-lg shadow-primary/20 transition-all flex items-center gap-2 active:scale-95"
                                        >
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="bg-gray-50 border-b border-gray-100">
                                    <th className="text-left py-4 px-6 text-xs font-bold text-gray-400 uppercase tracking-wider">Team Name</th>
                                    <th className="text-left py-4 px-6 text-xs font-bold text-gray-400 uppercase tracking-wider">Hackathon</th>
                                    ...
                                    <th className="text-left py-4 px-6 text-xs font-bold text-gray-400 uppercase tracking-wider">Result</th>
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {pastTeams.map(team => (
                                    <tr key={team.id} className="hover:bg-gray-50/50 transition-colors group">
                                        ...
                                        <td className="py-4 px-6 text-sm text-gray-600 font-medium">{team.hackathonName}</td>
                                        <td className="py-4 px-6 text-sm text-gray-500">{team.date}</td>
                                        <td className="py-4 px-6">
